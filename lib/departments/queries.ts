...
// Кандидаты в руководители департамента — ТОЛЬКО те, кто уже руководит
// хотя бы одним департаментом (managedDepartments: some) — по прямой
// просьбе Камилы: этот пикер предназначен для переназначения/добавления
// департамента уже существующему руководителю, не для превращения
// рядового сотрудника в руководителя впервые — для этого есть отдельный
// флоу (создание руководителя на /managers, см. lib/managers/actions.ts).
// Один человек может руководить несколькими департаментами одновременно
// (User.managedDepartments — Department[], не 0-1), поэтому уже
// назначенные руководители других отделов здесь по-прежнему видны. НЕ
// путать с getEmployeesForDepartmentAssignment() выше — та, наоборот,
// ИСКЛЮЧАЕТ действующих руководителей, потому что используется для
// другого пикера ("добавить рядового сотрудника").
export type ManagerCandidate = {
  id: string;
  fullName: string;
  position: string | null;
  // Названия департаментов, которыми этот человек УЖЕ руководит — здесь
  // всегда непусто (см. фильтр выше), но поле оставлено, чтобы показывать
  // "Руководит: X" в самом пикере.
  managedDepartmentNames: string[];
};

export async function getEmployeesForManagerAssignment(): Promise<ManagerCandidate[]> {
  const users = await prisma.user.findMany({
    where: { userType: UserType.ШТАТНЫЙ, managedDepartments: { some: {} } },
    select: {
      id: true,
      fullName: true,
      position: true,
