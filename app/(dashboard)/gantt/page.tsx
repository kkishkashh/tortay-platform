...
// (контрактного) срока раздела, толстая — текущего. Та же видимость, что
// у "Пульс недели" (см. hasPulseAccess) — строго члены департамента с
...
export default async function GanttPage() {
  ...
  const { projects, rangeStart, rangeEnd } = await getGanttData(session.user);
  ...
  if (!rangeStart || !rangeEnd) {
    return (
      <>
        ...
        <div className="p-8">
          <p className="text-sm text-muted-foreground">Пока нет разделов с заданными сроками.</p>
        </div>
      </>
    );
  }
  ...
  return (
    <>
      <PageHeader
        title="Гант"
        subtitle={`${formatShort(rangeStart)} — ${formatShort(rangeEnd)} · тонкая линия — базовый срок, толстая — текущий`}
      />
