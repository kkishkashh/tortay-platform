...
// Общий рендер месячной сетки + списка "Ближайшие сроки" — вынесен из
// /calendar (2026-08-06), чтобы переиспользовать и на вкладке "Загрузка и
// сроки" департамента (см. app/(dashboard)/departments/[id]/schedule-tab.tsx)
// без дублирования разметки. items — уже готовый список дедлайнов, откуда
// они взялись (личные задачи, задачи проекта, задачи департамента) этому
// компоненту не важно.
export function DeadlineCalendar({ items }: { items: CalendarDeadlineItem[] }) {
  ...
  const upcoming = [...items].sort((a, b) => a.deadline.getTime() - b.deadline.getTime());
        ...
        <CardContent className="space-y-3">
          ...
          <div className="grid grid-cols-7 gap-1 text-center text-xs text-muted-foreground">
            {days.map((day) => {
              ...
              const dayItems = [...items.filter((t) => isSameDay(t.deadline, day))].sort(
                (a, b) => PRIORITY_URGENCY_ORDER.indexOf(a.priority) - PRIORITY_URGENCY_ORDER.indexOf(b.priority),
              );
              const hasUrgent = dayItems.some((t) => t.priority === TaskPriority.СРОЧНЫЙ);
              const visibleDots = dayItems.slice(0, MAX_DOTS_PER_DAY);
              const hiddenCount = dayItems.length - visibleDots.length;
          ...
          <div className="space-y-2">
            {upcoming.map((item) => (
              <Link key={item.id} href={item.href} className="block">
                <Card size="sm" hoverable className={cn("border-l-4", TASK_PRIORITY_BORDER_COLOR[item.priority])}>
                  <CardContent className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                      <Badge variant={TASK_PRIORITY_BADGE_VARIANT[item.priority]}>
                        {TASK_PRIORITY_LABELS[item.priority]}
                      </Badge>
                      <span className="flex items-center gap-1 text-xs text-muted-foreground">
                        <CalendarClock className="size-3.5" />
                        {item.deadline.toLocaleDateString("ru-RU", { day: "2-digit", month: "short" })}
                      </span>
                    </div>
                  </CardContent>
                </Card>
